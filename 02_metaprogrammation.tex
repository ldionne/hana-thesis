...
une famille de types (comme \icpp{_type<...>}) sous une même banière. Ceci
...
banière \icpp{Type}. D'une certaine manière, on peut donc dire que
\[
...
définition rigoureuse de notre fonction \icpp{add_pointer} devient
\[
...
de la forme \icpp{_type<T>}. De plus, on peut se rendre compte que la notion
...
\icpp{int} est \icpp{int}, mais celui de n'importe quel \icpp{_type<T>} est
\icpp{Type}. Puisque la notion de type généralisé est plus générale, nous ne
...
donné que \icpp{IntegralConstant} ne peut que représenter des valeurs de
types intégraux (\icpp{int}, \icpp{long}, etc...), les types généralisés
auxquels il fait du sens d'appliquer \icpp{IntegralConstant} coïncident
