...
Une fonction $\icpp{f} : \icpp{std::string} \to \icpp{unsigned int}$ s'écrira
alors
\begin{cpp}
...
\footnote{
    En effet, le fichier \icpp{<string>} fournit une fonction \cc{operator+}
...
domaine est
\[
    \bigcup_{\text{tous les types \icpp{T}}} \icpp{_type<T>}
\]
...
\[
    \icpp{Type} = \bigcup_{\text{tous les types \icpp{T}}} \icpp{_type<T>}
\]
