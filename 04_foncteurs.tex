...
l'image de $T$ par le foncteur $F$. Cependant, le rôle de \icpp{transform}
demande un peu plus de justification. On notera que l'ordre des paramètres
de \icpp{transform} est fâcheux pour l'explication qui suit. C'est tout de
même cet ordre qui est utilisé dans Hana parce qu'il est avantageux lorsqu'on
veut passer une fonction anonyme C++ à \icpp{transform}. On commence donc par
échanger l'ordre des paramètres de \icpp{transform} en considérant plutôt la
...
\icpp{hana::_tuple<int, long, double>}
...
%%%%%%%%%%%%%%%%%%%%%% Tentative 2 avec des variants
On peut voir un tuple comme un array de taille fixe qui contient des variants.
Dans le fond,

    _tuple<T1, ..., Tn>

est grosso-modo la même chose que

    array<variant<T1, ..., Tn>, n>

Quand on voit un tuple de cette manière, il fait tout à coup du sens de
mapper une fonction sur le tuple; c'est la même chose que de mapper une
fonction dont le domaine serait le variant sur l'array. Ainsi, un tuple
peut être vu comme un Fonteur en considérant ...
    >> S'assurer qu'un array de variant est bien un foncteur et trouver une
    >> manière élégante de faire le lien avec un tuple.
...
[attention: ça veut dire que tuple<int, char> a le même ``type'' que tuple<char, int>]
\end{comment}
